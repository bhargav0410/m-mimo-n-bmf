...
void firstVector(cuFloatComplex* dY, cuFloatComplex* dH, cuFloatComplex* dX, int rows, int cols){
	...
	cudaMemcpy(Y, &dY[0], rows*cols*sizeof(*Y), cudaMemcpyHostToDevice);
	cudaDeviceSynchronize();
...
__global__ void doOneSymbol(cuFloatComplex* out, cuFloatComplex* Y, cuFloatComplex* Hconj, cuFloatComplex* Hsqrd,int rows1, int cols1){
	int rows = rows1;
	int cols= cols1;
...
void symbolPreProcess(cuFloatComplex *Y, cuFloatComplex *Hconj, cuFloatComplex *Hsqrd,int rows1, int cols1) {
	int rows = rows1;
	int cols= cols1;
	...
	cuFloatComplex* dY = 0;
	cudaMalloc((void**)&dY, rows*cols*lenOfBuffer*sizeof(*dY));
	cudaMemcpy(dY, Y, rows*cols*lenOfBuffer*sizeof(*Y), cudaMemcpyHostToDevice);
	cudaDeviceSynchronize();
	/*
	if(prefix>0){
		clock_t start, finish;
	...
	*/
	...
	for (int it = 1; it < lenOfBuffer; it++) {
		...
		cufftHandle plan;
		cufftPlan1d(&plan, cols, CUFFT_C2C, rows);
		cufftExecC2C(plan, (cufftComplex *)&dY[it*rows*cols], (cufftComplex *)&dY[it*rows*cols], CUFFT_FORWARD);
		
		if (it == 1) {
			std::string file = "FFT_Out.dat";
			cuFloatComplex* Yf;
			Yf = (cuFloatComplex*)malloc(rows*cols*sizeof(*Yf));
			cudaMemcpy(Yf, &dY[it*rows*cols], rows*cols*sizeof(*Yf), cudaMemcpyDeviceToHost);
			cudaDeviceSynchronize();
			std::cout << cudaGetErrorString(cudaGetLastError()) << std::endl;
			std::cout << "After FFT...\n";
			outfile.open(file.c_str(), std::ofstream::binary);
...
//				cuFloatComplex Yf_[rows*(cols+prefix)];
				...
				memcpy(Yf_, &dY[i*rows*cols], rows*(cols)*sizeof(*Yf_));
				//cudaDeviceSynchronize();
				//std::cout << cudaGetErrorString(cudaGetLastError()) << std::endl;
	...
	symbolPreProcess(dY, dH, dX, rows, cols);
	for(int i = 1; i < numberOfSymbolsToTest; i++) {	
		if(testEn){
			//printf("Symbol #%d:\n", i);
			//cuda copy it over
			memcpy(Yf, &dY[i*rows*cols], (cols-1)* sizeof (*Yf));
			if (i <= 1) {
