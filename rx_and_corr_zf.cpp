...
void copy_to_shared_mem(int chan, int fft_size, int cp_size, int num_syms) {
	//std::cout << "Here\n";
...
void LeastSquaresChanEst(int chan, int fft_size, int cp_size, int num_syms) {
	complexF* Y;
	Y = (complexF*)calloc(chan*fft_size, sizeof(*Y));
	fftOneRow(Y, fft_size, 0);
	//H (and Hconj) = 16x1023
	complexF* Hconj;
	Hconj = (complexF *)calloc(chan*(fft_size-1), sizeof (*Hconj));
	//X = 1x1023 -> later can become |H|^2
	complexF* X;
	X = (complexF *)calloc((fft_size-1), sizeof(*X));
	
	//Find H* (H conjugate) ->16x1023 and |H|^2 -> 1x1023
	std::string file = "ChanEst.dat";
	for (int iter = 0; iter < num_syms; iter++) {
		
		for (int j = 0; j < chan; j++) {
			memcpy(&Y[j*(fft_size)], &copy_buff[j][iter*(fft_size+cp_size) + cp_size], (fft_size)*sizeof(*Y));
		}
		
		firstVector(Y, Hconj, X, chan, fft_size, iter);
		std::cout << "Symbol: " << iter+1 << std::endl;
		if (iter == 0) {
		...
		outfile.write((const char*)Hconj, chan*(fft_size-1)*sizeof(*Hconj));
		outfile.close();
	}
	free(Y);
	free(Hconj);
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
	...
	int ctr = 0, cp_size, fft_size, num_APs, num_AP_ants;
    ...
    desc.add_options()
		...
		("fft-size", po::value<int>(&fft_size)->default_value(1024), "number of sub-carriers")
		("cp-size", po::value<int>(&cp_size)->default_value(72), "size of cyclic prefix")
		("num-aps", po::value<int>(&num_APs)->default_value(1), "number of APs")
		("num-ap-ants", po::value<int>(&num_AP_ants)->default_value(16), "number of antennas per AP")
    ;
    po::variables_map vm;
	...
	*/
    ...
    std::vector<std::vector<std::complex<float> > > buff1(
        channel_nums.size(), std::vector<std::complex<float> >(2*num_samps)
    );
	...
	int samps_per_buff = 2*num_samps, length = 0;//, iter = 0;
...
mylabel:
		...
		*/
		...
		if (corr_flag == true) { // and first_time == true) {
			for (int ch = 0; ch < channel_nums.size(); ch++) {
				int j = 0;
				for (int i = length + pn_buff.size(); i < length + pn_buff.size() + num_samps; i++) {
		...
		if (copy_flag == true) {
			...
			int syms = num_AP_ants*num_APs;
			//nt temp_cp_size = 0;
			LeastSquaresChanEst(ch, fft_size, cp_size, syms);
		//	t[0] = boost::thread(copy_to_shared_mem, boost::ref(ch), boost::ref(fft_size), boost::ref(cp_size), boost::ref(syms));
		//	t[1] = boost::thread(LeastSquaresChanEst, boost::ref(ch), boost::ref(fft_size), boost::ref(temp_cp_size), boost::ref(syms));
		}
		std::cout << "Here...\n";
	//	t[0].join();
	//	t[1].join();
